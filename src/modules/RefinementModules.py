...
class RefinementModule(pl.LightningModule):
    """
    Defines a generic refinement module. The module is expected to have a forward method that takes a batch of data in dictionary format,
    containing the extracted features and the coarse predictions, and returns the batch. It needs to implement a calculate_loss method.
    """
...
class Identity(RefinementModule):
    """
    Defines a refinement module that does nothing. This is useful for testing the feature extraction module in isolation.
    """
    def __init__(self):
        super().__init__()
...
class FeatureTransformer(nn.Module):
    ...
    def calculate_metrics(self, batch, mode):
        ...
        offsets_magnitude_mean, offsets_magnitude_std = offsets_magnitude.mean(), offsets_magnitude.std()

        #Mask the offsets with the loss mask
        offsets_masked = offsets_magnitude * loss_mask
        offsets_masked_mean, offsets_masked_std = offsets_masked.mean(), offsets_masked.std()
...
class PatchTransformer(nn.Module):
    """
    Enrich the coarse features with patch features extracted with a simple CNN regressor, then refine the predictions using a transformer.
    """
    def __init__(
        self,
        n_landmarks: int,
    ...
    def forward(self, batch):
        ...
        if self.warmup_epochs > 0 and self.training and self.current_epoch < self.warmup_epochs:
            ...
            coarse_preds = self.current_epoch / self.warmup_epochs * coarse_preds + (1 - self.current_epoch / self.warmup_epochs) * batch['target']
