...
sys.path.append('/home/daniel/Documents/Uni/MT/poi-prediction/src/')

from utils.misc import surface_project_coords
from modules.PoiModule import PoiPredictionModule
from src.modules.PoiDataModules import POIDataModule, JointDataModule
...
def np_to_ctd(t, vertebra, origin, rotation, idx_list = None, shape = (128, 128, 96), zoom = (1,1,1), offset = (0,0,0)):
    ctd = {}
...
def create_prediction_poi_files(data_module_save_path, checkpoint_path, poi_file_ending, split='val', joint=False, save_in_dir=False, save_path=None, return_paths=False, project = True):
    ...
    for batch in val_dl:
        ...
        #Get target, projected refined_preds, target_indices, loss_mask, offset and poi_path
        ...
        for sub, vert, preds, indices, poi_path, offset in zip(subject_batch, vertebra_batch, pred_batch, target_indices_batch, poi_path_batch, offset_batch):
            ...
            ctd = np_to_ctd(preds, vert, origin, rotation, idx_list = indices, shape = shape, zoom = zoom, offset = offset)
            ...
            if save_in_dir:
                ...
                if ctd_save_path == poi_path:
                    #Print warning
                    print(f"Warning: The save path {ctd_save_path} is the same as the original POI path. The new file will be saved with the ending '_pred.json'")
...
def create_self_training_pois(data_module_save_path, checkpoint_path, poi_file_ending, split='val', joint=False, thre=3.0):
    new_bad_pois = {
        'subject': [],
        'vertebra': [],
        'bad_poi_list': [],
    ...
    for batch in val_dl:
        ...
        target_projected_batch, _ = surface_project_coords(target_batch, batch['surface'])
        ...
        for sub, vert, preds, indices, poi_path, offset, loss_mask, target in zip(subject_batch, vertebra_batch, refined_preds_projected_batch, target_indices_batch, poi_path_batch, offset_batch, loss_mask_batch, target_projected_batch):
            #Open the old POI file to get the origin and rotation
            ctd = POI.load(poi_path)
            origin = ctd.origin
            rotation = ctd.rotation
            ...
            #Where the loss mask is false (i.e. a bad gt), we use the predicted POI as the new POI
            pred_mask = np.logical_not(loss_mask)

            #If the distance between predicted and target is larger than the threshold, we mark the POI as bad
            ...
            #Create the new POI file
            ctd = np_to_ctd(new_target, vert, origin, rotation, idx_list = indices, shape = shape, zoom = zoom, offset = offset)
...
def run_predictions(data_module_save_path, checkpoint_path, split='val', joint = False, alternative_poi_ending=None):
    #Change the ending of the POI files if necessary
    data_module = load_data_module_from_config(data_module_save_path, joint=joint, alternative_poi_ending = alternative_poi_ending)
    data_module.setup()
    ...
    for batch in val_dl:
        ...
        keys = ['target', 'target_indices', 'coarse_preds', 'refined_preds', 'coarse_preds_projected', 'coarse_pred_proj_distances', 'refined_preds_projected', 'refined_preds_proj_distances', 'loss_mask', 'subject', 'vertebra']

        for values in zip(target_batch, target_indices_batch, coarse_preds_batch, refined_preds_batch, coarse_preds_projected_batch, coarse_pred_proj_distances_batch, refined_preds_projected_batch, refined_preds_proj_distances_batch, loss_mask_batch, subject_batch, vertebra_batch):
            data_dict = dict(zip(keys, values))
            #Iterate over all POIs to collect POI-wise information
            for poi_idx, t, c, r, c_proj, r_proj, c_proj_dist, r_proj_dist, l in zip(data_dict['target_indices'], data_dict['target'], data_dict['coarse_preds'], data_dict['refined_preds'], data_dict['coarse_preds_projected'], data_dict['refined_preds_projected'], data_dict['coarse_pred_proj_distances'], data_dict['refined_preds_proj_distances'], data_dict['loss_mask']):
...
def create_prediction_df(data_module_save_path, checkpoint_path, split='val', joint=False, alternative_poi_ending=None):
    pred_dict = run_predictions(data_module_save_path, checkpoint_path, split, joint, alternative_poi_ending)
    #Calculate distances between target and predicted POIs
    pred_dict['coarse_error'] = [np.linalg.norm(np.array(t) - np.array(c)) for t, c in zip(pred_dict['target'], pred_dict['coarse'])]
    pred_dict['refined_error'] = [np.linalg.norm(np.array(t) - np.array(r)) for t, r in zip(pred_dict['target'], pred_dict['refined'])]

    #Calculate distances between target and projected POIs
    pred_dict['coarse_proj_error'] = [np.linalg.norm(np.array(t) - np.array(c)) for t, c in zip(pred_dict['target'], pred_dict['coarse_proj'])]
    pred_dict['refined_proj_error'] = [np.linalg.norm(np.array(t) - np.array(r)) for t, r in zip(pred_dict['target'], pred_dict['refined_proj'])]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_module_save_path', type=str, help='Path to the saved DataModule configuration')
    ...
    parser.add_argument('--joint', action='store_true', help='Whether to use the JointDataModule')

    args = parser.parse_args()

    df = create_prediction_df(args.data_module_save_path, args.checkpoint_path, args.split, args.joint)
